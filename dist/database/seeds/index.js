...
async function run() {
    ...
    `);
        ...
        `, [(0, ulid_1.ulid)(), category.name, category.slug, category.sortOrder]);
        }
        const seededUsers = [];
        ...
        const merchant = await dataSource.query(`SELECT id FROM merchants LIMIT 1`);
        const merchantId = merchant[0]?.id || (0, ulid_1.ulid)();
        const electronicsCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'electronics' LIMIT 1`);
        const electronicsId = electronicsCat[0]?.id || (0, ulid_1.ulid)();
        const fashionCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'fashion' LIMIT 1`);
        const fashionId = fashionCat[0]?.id || (0, ulid_1.ulid)();
        const homeGardenCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'home-garden' LIMIT 1`);
        const homeGardenId = homeGardenCat[0]?.id || (0, ulid_1.ulid)();
        const beautyCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'beauty' LIMIT 1`);
        const beautyId = beautyCat[0]?.id || (0, ulid_1.ulid)();
        const sportsCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'sports' LIMIT 1`);
        const sportsId = sportsCat[0]?.id || (0, ulid_1.ulid)();
        const foodGroceryCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'food-grocery' LIMIT 1`);
        const foodGroceryId = foodGroceryCat[0]?.id || (0, ulid_1.ulid)();
        const booksCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'books' LIMIT 1`);
        const booksId = booksCat[0]?.id || (0, ulid_1.ulid)();
        const toysKidsCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'toys-kids' LIMIT 1`);
        const toysKidsId = toysKidsCat[0]?.id || (0, ulid_1.ulid)();
        const toolsDiyCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'tools-diy' LIMIT 1`);
        const toolsDiyId = toolsDiyCat[0]?.id || (0, ulid_1.ulid)();
        const petSuppliesCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'pet-supplies' LIMIT 1`);
        const petSuppliesId = petSuppliesCat[0]?.id || (0, ulid_1.ulid)();
        const healthCat = await dataSource.query(`SELECT id FROM categories WHERE slug = 'health' LIMIT 1`);
        const healthId = healthCat[0]?.id || (0, ulid_1.ulid)();
        const products = [
            {
        ...
        if (customerId && merchantId) {
            const addressRow = await dataSource.query(`SELECT id FROM addresses WHERE user_id = $1 ORDER BY is_default DESC LIMIT 1`, [customerId]);
            const addressId = addressRow[0]?.id;
            const merchantProducts = await dataSource.query(`SELECT id, name, images, price FROM products WHERE merchant_id = $1 ORDER BY created_at ASC LIMIT 6`, [merchantId]);
            if (addressId && merchantProducts.length >= 3) {
                ...
                for (const seed of orderSeeds) {
                    const subtotal = seed.items.reduce((sum, item) => sum + Number(item.product.price || 0) * item.quantity, 0);
        ...
        `);
          ...
          `, [(0, ulid_1.ulid)(), orders[0].id, orders[0].customer_id, orders[0].merchant_id]);
                await dataSource.query(`
            INSERT INTO disputes (
              id, order_id, customer_id, merchant_id, reason,
          ...
          `, [
            ...
            `, [(0, ulid_1.ulid)(), adminId, log.action, log.type, log.target, JSON.stringify({ ip: '127.0.0.1', timestamp: new Date().toISOString() })]);
                }
            }
        }
        console.log('Seed complete: database reset and reloaded.');
        console.table(seededUsers.map((user) => ({
            role: user.role,
            email: user.email,
            password: user.password,
        })));
